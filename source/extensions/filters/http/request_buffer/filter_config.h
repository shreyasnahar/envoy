...
using ProtoBufferBehavior =
    envoy::extensions::filters::http::request_buffer::v3::BufferBehavior;
using ProtoStreamConfig = envoy::extensions::filters::http::request_buffer::v3::StreamConfig;
using ProtoRequestBufferFilterConfig =
...
// This is intentionally unusable; filter will generate a RequestBufferFilterMergedConfig
// to merge the global config and any route/vhost configs.
class RequestBufferFilterConfig : public Router::RouteSpecificFilterConfig {
public:
  explicit RequestBufferFilterConfig(
  ...
  const ProtoRequestBufferFilterConfig& proto() const { return config_; }
...
private:
  ...
  const ProtoRequestBufferFilterConfig config_;
};
...
using FilterConfigVector =
    absl::InlinedVector<std::reference_wrapper<const RequestBufferFilterConfig>, 4>;

class RequestBufferFilterMergedConfig {
public:
  class StreamConfig {
  public:
  ...
  explicit RequestBufferFilterMergedConfig(const FilterConfigVector& configs);
