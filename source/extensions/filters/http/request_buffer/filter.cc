...
RequestBufferFilter::RequestBufferFilter(
    std::shared_ptr<RequestBufferFilterConfig> base_config)
    : base_config_(std::move(base_config)) {}

const std::string& RequestBufferFilter::filterName() {
  CONSTRUCT_ON_FIRST_USE(std::string, "envoy.filters.http.request_buffer");
}

bool RequestBufferFilter::initPerRouteConfig() {
  if (config_) {
    return true;
  }
  ...
  if (route) {
    ...
    for (const auto* route_cfg : route->perFilterConfigs(RequestBufferFilter::filterName())) {
      auto typed_cfg = dynamic_cast<const RequestBufferFilterConfig*>(route_cfg);
      if (typed_cfg) {
  ...
  if (config_->request().behavior().bypass()) {
...
Http::FilterHeadersStatus RequestBufferFilter::onBadConfig() {
  filterError("config was missing manager_config");
  return Http::FilterHeadersStatus::StopIteration;
}

Http::FilterHeadersStatus RequestBufferFilter::decodeHeaders(Http::RequestHeaderMap& headers,
                                                                bool end_stream) {
...
void RequestBufferFilter::setDecoderFilterCallbacks(Http::StreamDecoderFilterCallbacks& callbacks) {
  request_callbacks_ = &callbacks;
}

void RequestBufferFilter::filterError(absl::string_view err) {
  aborted_ = true;
...
Http::FilterDataStatus RequestBufferFilter::decodeData(Buffer::Instance& data, bool end_stream) {
...
Http::FilterDataStatus RequestBufferFilter::receiveData(BufferedStreamState& state,
...
Http::FilterTrailersStatus RequestBufferFilter::receiveTrailers(const BufferBehavior& behavior,
...
Http::FilterTrailersStatus RequestBufferFilter::decodeTrailers(Http::RequestTrailerMap&) {
  return receiveTrailers(request_state_.config_->behavior(), request_state_);
}

void RequestBufferFilter::onDestroy() {
  *is_destroyed_ = true;
...
void RequestBufferFilter::maybeOutputRequest() {
...
void BufferedStreamState::setConfig(
    const RequestBufferFilterMergedConfig::StreamConfig& config) {
  config_ = &config;
}
...
bool RequestBufferFilter::maybeSendWatermarkUpdates() {
...
void RequestBufferFilter::onStateChange() {
  if (aborted_) {
    return;
  }
  maybeOutputRequest();
...
bool RequestBufferFilter::maybeStorage(BufferedStreamState& state,
...
absl::AnyInvocable<void(absl::Status)> RequestBufferFilter::getOnFileActionCompleted() {
...
void RequestBufferFilter::safeDispatch(absl::AnyInvocable<void()> fn) {
...
void RequestBufferFilter::dispatchStateChanged() {
