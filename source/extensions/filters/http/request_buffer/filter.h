...
#include "source/extensions/filters/http/request_buffer/fragment.h"
...
struct BufferedStreamState {
  const RequestBufferFilterMergedConfig::StreamConfig* config_;
  ...
  void setConfig(const RequestBufferFilterMergedConfig::StreamConfig& config);
  void close();
};

class RequestBufferFilter : public Http::StreamDecoderFilter,
                               public Logger::Loggable<Logger::Id::http2> {
public:
  explicit RequestBufferFilter(std::shared_ptr<RequestBufferFilterConfig> base_config);
...
private:
  ...
  // by the time it pops off the dispatcher queue.
  ...
  const std::shared_ptr<RequestBufferFilterConfig> base_config_;
  // config_ is a merge of the base config and any per-route config.
  absl::optional<RequestBufferFilterMergedConfig> config_;
  Http::StreamDecoderFilterCallbacks* request_callbacks_ = nullptr;
  bool aborted_ = false;

  CancelFunction cancel_in_flight_async_action_;

  friend class RequestBufferFilterConfigTest; // to inspect base_config_
  friend class RequestBufferFilterTest;       // to inspect config_
};
