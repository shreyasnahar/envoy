#include "source/extensions/filters/http/request_buffer/filter_config.h"
...
const BufferBehavior& selectBufferBehavior(const ProtoBufferBehavior& behavior) {
  ...
  throw EnvoyException("invalid BufferBehavior in RequestBufferFilterConfig");
}
...
size_t getMemoryBufferBytesLimit(const StreamConfigVector& configs) {
  ...
  return RequestBufferFilterMergedConfig::default_memory_buffer_bytes_limit;
}
...
size_t getStorageBufferBytesLimit(const StreamConfigVector& configs) {
  ...
  return RequestBufferFilterMergedConfig::default_storage_buffer_bytes_limit;
}
...
absl::string_view getStorageBufferPath(const FilterConfigVector& configs) {
  for (const RequestBufferFilterConfig& cfg : configs) {
...
const RequestBufferFilterMergedConfig::StreamConfig
getBufferRequestConfig(const FilterConfigVector& configs, bool has_file_manager) {
  ...
  return RequestBufferFilterMergedConfig::StreamConfig(request_configs, has_file_manager);
}

} // namespace

RequestBufferFilterMergedConfig::StreamConfig::StreamConfig(const StreamConfigVector& configs,
...
RequestBufferFilterConfig::RequestBufferFilterConfig(
  ...
  if (!config_.storage_buffer_path().value().empty()) {
    ...
    if (stat(config_.storage_buffer_path().value().c_str(), &s) != 0 ||
      ...
      throw EnvoyException(
          absl::StrCat("RequestBufferFilterConfig: configured storage_buffer_path '",
                       config_.storage_buffer_path().value(), "' is not a directory"));
    }
  }
}

RequestBufferFilterMergedConfig::RequestBufferFilterMergedConfig(
