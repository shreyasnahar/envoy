...
/**
 * A config validator extension that validates that WAF has the envoy config version.
 */
class AzureWafListenerValidator : public Envoy::Config::ConfigValidator {
public:
  AzureWafListenerValidator(
      const envoy::extensions::config::validators::azure_waf_listener::v3::AzureWafListenerValidator&
          config)
      : config_(config){
...
private:
  envoy::extensions::config::validators::azure_waf_listener::v3::AzureWafListenerValidator config_;
